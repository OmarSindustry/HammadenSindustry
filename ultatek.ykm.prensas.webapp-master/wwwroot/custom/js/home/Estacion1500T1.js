...
$(document).ready(function () {
    ...
    $('#datetimepicker').on('apply.daterangepicker', function (ev, picker) {
        obtenerDatosPorFecha(picker.startDate.format('YYYY-MM-DD HH:mm'), picker.endDate.format('YYYY-MM-DD HH:mm'));
    });

    // Inicializar la gráfica
    var chart = echarts.init(document.getElementById('produccion_actual'));
    var actual_options = {
        title: {
            text: 'Consumo KWH',
        },
    ...
    function obtenerDatosPorFecha(fechaInicial, fechaFinal) {
        $.ajax({
            type: "GET",
            url: "/Dashboard/GetConsumoByFecha", // Ensure this matches your API route
            data: {
            ...
            success: function (response) {
                if (response && response.length > 0) {
                    let fechas = response.map(d => moment(d.fecha).format('DD-MM-YYYY HH:mm'));
                    let consumos = response.map(d => d.consumoKWH);
    ...
    // Inicializar con los datos del día actual
    let inicio = moment().startOf('day').format('YYYY-MM-DD HH:mm');
    let fin = moment().endOf('day').format('YYYY-MM-DD HH:mm');
    obtenerDatosPorFecha(inicio, fin);
});
var produccion_actual = echarts.init(produccion_actual_element);

var stamping_TRF1500_1_actual_element = document.getElementById('produccion_actual');
var stamping_TRF1500_1_actual_options = {
...
var stamping_TRF1500_1_actual = echarts.init(stamping_TRF1500_1_actual_element);
...
function getTRF1500_1_actual() {

    $.ajax({

        type: "GET",
        url: "/Home/Get_actual_trf1500_1_Json",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            if (response != null) {

                stamping_TRF1500_1_actual_options = {
                ...
                for (var i = 0, l = part_number.length; i < l; i++) {
                    ...
                    var data = [];
                    stamping_TRF1500_1_actual_options.series[i].name = a[i].part_number
                    ...
                    for (var k = 0, m = fecha.length; k < m; k++) {
                        ...
                        stamping_TRF1500_1_actual_options.series[i].data = data;
                ...
                for (var k = 0, m = stamping_TRF1500_1_actual_options.series.length; k < m; k++) {
                    if (stamping_TRF1500_1_actual_options.series[k].data.length != 0) {

                        for (var i = 0, n = stamping_TRF1500_1_actual_options.series.length; i < n; i++) {
                            if (stamping_TRF1500_1_actual_options.series[k].data[i] !== undefined) {
                                
                                totales[i] = totales[i] + + stamping_TRF1500_1_actual_options.series[k].data[i];
                            }                            
                        }
                ...
                $.ajax({
                    ...
                    type: "GET",
                    url: "/Home/Get_proyectado_trf1500_1_Json",
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
                        if (response != null) {
                ...
                stamping_TRF1500_1_actual_options.series[36].data = [ 
                ...
                stamping_TRF1500_1_actual_options.series[37].data = [
                    proyectado_pasado,
                    proyectado_pasado * 2,
                    proyectado_pasado * 3,
                    proyectado_pasado * 4,
                ...
                stamping_TRF1500_1_actual_options.series[38].data = totales;


                stamping_TRF1500_1_actual_options.xAxis[0].data = fecha;
                stamping_TRF1500_1_actual_options.legend.data = part_numbers;
                stamping_TRF1500_1_actual.setOption(stamping_TRF1500_1_actual_options, true);
...
function get_jph() {
    ...
    $.ajax({
        ...
        type: "GET",
        url: "/Home/Get_jph_trf1500_1_Json"
...
function get_grafica() {
    ...
    $.ajax({
        ...
        type: "GET",
        url: "/Dashboard/Dashboard_Grafica_TRF1500_1_Json",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            if (response != null) {
